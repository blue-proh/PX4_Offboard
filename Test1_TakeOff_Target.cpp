...
/************************************************************************************
 *本程序用于无人机OFFBOARD模式下在Gazebo中的起降测试，起飞高度自行设置
 *Li Zhiheng
 *2021.03.28
 *************************************************************************************/
...
int main(int argc, char **argv) {
    ...
    ros::init(argc, argv, "Test1_TakeOff_Target");
    ros::NodeHandle nh;
    ...
    while (ros::ok()) {
        ...
        //无人机起飞与着陆
        if (Task_step == 1) {
            if (current_state.armed && !UAV_Reach_H_Flag && current_position.pose.pose.position.z > 0.95) {
                ROS_INFO("UAV reach target height");
                UAV_Reach_H_Flag = 1; //无人机达到预定高度
                last_request = ros::Time::now();
            }
            if (UAV_Reach_H_Flag && ros::Time::now() - last_request > ros::Duration(10.0) && current_state.mode == "OFFBOARD") {

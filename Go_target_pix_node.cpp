...
int main(int argc, char **argv) {
    ...
    ros::init(argc, argv, "Go_target_pix_node");
    ros::NodeHandle nh;
    ...
    while (ros::ok()) {
        ...
        {
            /*
            if(t265_vision_pose.pose.position.z < 0.7) //高度低于0.5且位置正确时转为降落模式
                Task_step = 99;*/
            ...
            if(!Lost_Object_Flag) {
                ...
                //如果位置正常，开始降落
                if(Err_x < 100 && Err_y < 100 && Err_x > -100 && Err_y > -100 && (ros::Time::now() - last_request > ros::Duration(4.0)))
				{
                    //Target_Height = t265_vision_pose.pose.position.z; //正常降落时目标高度为当前高度
                    //vel.twist.linear.z = -0.15;
                    Task_step = 99;
				}
                else {
                    vel.twist.linear.z = Err_z; //位置不正常时，在当前高度悬停
                    if(!(Err_x < 100 && Err_y < 100 && Err_x > -100 && Err_y > -100))
                    last_request = ros::Time::now();
                }
            }
            else

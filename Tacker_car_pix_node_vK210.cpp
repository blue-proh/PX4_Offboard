...
#include <k210_target/BoundingBox.h> // k210地面目标位置
/************************************************************************************
...
/******************K210**********************
在机头向前的情况下
摄像头向前移动，Target_pose.center_y增大
摄像头向左移动，Target_pose.center_x增大
**************************************************/
void Calculate_Target_pose()//用于找到目标之后，计算无人机与目标的偏差
{
    //相机坐标系转换到无人机坐标系
    Pix_Target_x = Target_pose.center_y;
    Pix_Target_y = Target_pose.center_x;
    Wall_time = Target_pose.wall_time;
    ROS_INFO_STREAM("Pix_Target_x = " << Pix_Target_x << ", " << "Pix_Target_y = " << Pix_Target_y << ", " << "Wall_time = " << Wall_time);

    //计算水平方向err
    //Err_pos_x = Pix_Target_x - Image_Height / 2.0;
    //Err_pos_y = Pix_Target_y - Image_Width / 2.0;
    Err_pos_x = Image_Height / 2.0 - Pix_Target_x;
    Err_pos_y = Image_Width / 2.0 - Pix_Target_y;
    //计算高度方向err
...
int main(int argc, char **argv) {
    ...
    ros::init(argc, argv, "Tacker_car_pix_node_vK210");
    ros::NodeHandle nh;
    ...
    while (ros::ok()) {
        ...
        if (Task_step == 2)
        {
            ...
            if(Err_pos_x != (Image_Height / 2.0) && Err_pos_y != (Image_Height / 2.0)) //无人机在巡航的过程中找到目标
            {
                last_request = ros::Time::now();
                Find_Object_Flag = 1;
        ...
        if (Task_step == 3)
        {
            ...
            Calculate_Target_pose();
            if(Err_pos_x == (Image_Height / 2.0) && Err_pos_y == (Image_Height / 2.0))
            {
            ...
            //速度控制
            if(!Lost_Object_Flag) {
                target_vel.twist.linear.x = Err_pos_x / 600.0;
                target_vel.twist.linear.y = Err_pos_y / 600.0;
            ...
            else
            {
                ...
                target_vel.twist.linear.x = Log_Err_pos_x / 600.0;
                target_vel.twist.linear.y = Log_Err_pos_y / 600.0;
                target_vel.twist.linear.z = Err_pos_z / 1.0;
                last_request = ros::Time::now();
            }
